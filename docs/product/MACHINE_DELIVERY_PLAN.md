...
| Active feature | — | No active Spec Kit feature; `005-model-b-workflow` acceptance is reopened below |
...
| 2 | **TODO** | `machine-model-b-workflow` | 1, C1, C2 | A macOS user imports and remembers compatible OS/language ROM assignments, boots Model B, sees continuous completed frames, types/runs the M1 BASIC program and uses accessible run, pause, reset and BREAK controls with actionable diagnostics. Implementation and automated evidence exist, but direct typed-program and recoverable-failure observations remain open. |
