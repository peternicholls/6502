...
- [X] T008 Build and launch `BeebDemo-macOS`; perform OS/language import, bank-12 BASIC boot, the physical-key program, two-frame observation, run/pause/reset/BREAK and one recoverable failure; record keyboard and assistive observation in `specs/005-model-b-workflow/evidence/macos-application-observation.md`; commit observed acceptance.
