...
- **FR-003**: The frame queue MUST have capacity three, dequeue the oldest retained frame, and on overflow discard exactly the oldest unconsumed frame before enqueuing the newest; every discard increments a monotonic dropped-frame counter.
...
- **FR-005**: The audio ring MUST preserve FIFO order, target a 2,048-sample fill, report `max(0, 2048 - available)` demand, discard the oldest samples on overflow while incrementing an exact overrun count, and return all available samples plus the exact shortfall on underrun.
