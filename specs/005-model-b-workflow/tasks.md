...
- [ ] T002 Add `Tests/ModelBWorkflow/test-ci-contract.sh` and update `.github/workflows/ci.yml` only if a focused hosted gate is necessary; observe the test fail first, verify its source contract, then commit the CI decision checkpoint.
...
- [ ] T010 [US1] Implement read-only security-scoped bookmark persistence, stale refresh, balanced access and private byte loading in `Sources/BeebDemo/main.swift` plus any required macOS configuration under `Beeb6502.xcodeproj/`; pass T008 and the changed macOS build, then commit the host firmware checkpoint.
...
- [ ] T016 [US2] Document the lawful observation fixture and BASIC journey in `specs/005-model-b-workflow/contracts/machine-workflow.md` and `specs/005-model-b-workflow/quickstart.md`; run focused documentation/build probes and commit the checkpoint explicitly closing US2.
...
- [ ] T017 [US3] Add red lifecycle, reset/BREAK epoch and repeated-control recovery tests in `Tests/BeebKitTests/BeebMachineTests.swift` and the relevant `Tests/C2/test-output-*.sh` script; observe failure and commit the red control-boundary checkpoint.
...
T002 may run after T001 and may be omitted when existing CI already proves the new aggregate. T007/T008, T012/T013 and T017/T018 are parallel opportunities after their foundation, but their dependent tasks remain sequential.
