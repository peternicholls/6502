...
void SN76489::write(std::uint8_t value) {
    if ((value & 0x80) != 0) {
        ...
        if (latchedVolume_) {
            ...
            tone_[latchedChannel_] = static_cast<std::uint16_t>((tone_[latchedChannel_] & 0x3F0) | (value & 0x0F));
    ...
    if (latchedVolume_) {
        ...
        tone_[latchedChannel_] = static_cast<std::uint16_t>(((value & 0x3F) << 4) | (tone_[latchedChannel_] & 0x0F));
        if (tone_[latchedChannel_] == 0) tone_[latchedChannel_] = 1;
    }
}
...
void SN76489::render(float* mono, std::size_t frames, double sampleRate) {
    ...
    for (std::size_t frame = 0; frame < frames; ++frame) {
        ...
        const double noiseFrequency = rate == 3
            ? chipClock / (32.0 * std::max<std::uint16_t>(1, tone_[2]))
