...
void CPU6502::setFlag(Flag f, bool value) noexcept {
    if (value)
        ...
        p_ &= static_cast<std::uint8_t>(~f);
    p_ |= Unused;
}

std::uint8_t CPU6502::fetch8() {
    return read(pc_++);
}
...
std::uint16_t CPU6502::zp() {
    return fetch8();
}
std::uint16_t CPU6502::zpx() {
    return static_cast<std::uint8_t>(fetch8() + x_);
}
std::uint16_t CPU6502::zpy() {
    return static_cast<std::uint8_t>(fetch8() + y_);
}
std::uint16_t CPU6502::abs() {
    return fetch16();
}
...
void CPU6502::sbc(std::uint8_t value) {
    ...
    auto low =
        static_cast<int>(a_ & 0x0F) - static_cast<int>(value & 0x0F) - static_cast<int>(borrow);
    if (low < 0) low -= 6;
...
[[noreturn]] void CPU6502::illegal(std::uint8_t opcode, std::uint16_t address) const {
    ...
    message << "unsupported NMOS 6502 opcode $" << std::hex << std::uppercase << std::setw(2)
            << std::setfill('0') << static_cast<unsigned>(opcode) << " at $" << std::setw(4)
            << address;
...
std::uint32_t CPU6502::step() {
    ...
    case 0x06:
        address = zp();
        rmw(address, [&](auto v) { return asl(v); });
        return finish(5);
    case 0x08:
        push(static_cast<std::uint8_t>(p_ | Break | Unused));
        return finish(3);
    ...
    case 0x0E:
        ...
        return finish(6);
    case 0x10:
        return finish(branch(!flag(Negative)));
    case 0x11:
        address = indy(crossed);
        a_ |= readOp(address);
        setNZ(a_);
        return finish(5 + crossed);
    case 0x15:
        a_ |= readOp(zpx());
        setNZ(a_);
        return finish(4);
    ...
    case 0x18:
        ...
        return finish(2);
    case 0x19:
        address = absy(crossed);
        a_ |= readOp(address);
        setNZ(a_);
        return finish(4 + crossed);
    case 0x1D:
        address = absx(crossed);
        a_ |= readOp(address);
        setNZ(a_);
        return finish(4 + crossed);
    case 0x1E:
    ...
    case 0x26:
        address = zp();
        rmw(address, [&](auto v) { return rol(v); });
        return finish(5);
    case 0x28:
    ...
    case 0x40:
        p_ = static_cast<std::uint8_t>((pull() | Unused) & ~Break);
        pc_ = pull();
        pc_ |= static_cast<std::uint16_t>(pull()) << 8;
        return finish(6);
    case 0x41:
        a_ ^= readOp(indx());
        setNZ(a_);
    ...
    case 0x46:
        address = zp();
        rmw(address, [&](auto v) { return lsr(v); });
        return finish(5);
    case 0x48:
        push(a_);
        return finish(3);
    ...
    case 0x4E:
        ...
        return finish(6);
    case 0x50:
        return finish(branch(!flag(Overflow)));
    case 0x51:
        address = indy(crossed);
        a_ ^= readOp(address);
        setNZ(a_);
    ...
    case 0x66:
        address = zp();
        rmw(address, [&](auto v) { return ror(v); });
        return finish(5);
    case 0x68:
        a_ = pull();
        setNZ(a_);
    ...
    case 0x8E:
        ...
        return finish(4);
    case 0x90:
        return finish(branch(!flag(Carry)));
    case 0x91:
        address = indy(crossed);
        write(address, a_);
        return finish(6);
    ...
    case 0xC0:
        ...
        return finish(2);
    case 0xC1:
        compare(a_, readOp(indx()));
        return finish(6);
    case 0xC4:
        compare(y_, readOp(zp()));
        return finish(3);
    case 0xC5:
        compare(a_, readOp(zp()));
        return finish(3);
    case 0xC6:
        address = zp();
        rmw(address, [&](auto v) {
    ...
    case 0xCA:
        ...
        return finish(2);
    case 0xCC:
        compare(y_, readOp(abs()));
        return finish(4);
    case 0xCD:
        compare(a_, readOp(abs()));
        return finish(4);
    case 0xCE:
        address = abs();
        rmw(address, [&](auto v) {
        ...
        return finish(6);
    case 0xD0:
        return finish(branch(!flag(Zero)));
    case 0xD1:
        address = indy(crossed);
        compare(a_, readOp(address));
        return finish(5 + crossed);
    case 0xD5:
        compare(a_, readOp(zpx()));
        return finish(4);
    case 0xD6:
        address = zpx();
        rmw(address, [&](auto v) {
