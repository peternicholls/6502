...
assert_generated_contains() {
    local text="$1"
    if ! rg -Fq --glob '*.html' -- "${text}" "${repo_root}/.build/docs/cpp"; then
        printf 'generated C/C++ documentation is missing required text: %s\n' "${text}" >&2
        return 1
    fi
}

assert_generated_contains "beeb_create_with_profile"
assert_generated_contains "MachineRuntime"
assert_generated_contains "operation-owned"
